...
opts =
  flip info fullDesc $
  switch
    (long "trial" <> help "Run a quick version of benchmarks to check that all is working correctly")
...
smcResults =
  map
    (\p ->
       Vector.replicateM 10 $
       fmap HMM.hmmKL $ explicitPopulation $ smcMultinomial (length HMM.values) p HMM.hmm)
    smcParams
...
hmmBenchmark = do
  ...
  liftIO $
    toFile (fo_format .~ PDF $ def) "smc.pdf" $ do
      layout_title .= "HMM"
      oneShotPlot "#particles" "KL" [("SMC", zip smcParamsDouble (map (errBars 2) smcRes))]
