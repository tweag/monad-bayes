...
-- dpClusters :: (MonadInfer m, Double ~ Double) => m Int
...
-- -- cluster_param :: (MonadSample m, Double ~ Double) => Int -> m (Double, Double, Double)
-- -- cluster_param _ = do
-- --   b <- beta 1 1
-- --   var <- do
-- --     t <- gamma a b
...
-- --   return (b, mean, var)
-- --
-- -- dpMem :: (MonadInfer m, Double ~ Double) => m [Int]
...
-- --
-- -- dpMemClusters :: (MonadInfer m, Double ~ Double) => m Int
...
-- posteriorClustersDist :: (MonadSample m, Double ~ Double) => m Int
-- posteriorClustersDist = categorical $ zip ns $ map lookup ns
--   where
--     ns = [1 .. length obs]
...
-- -- type instance CustomReal (StateCache c m) = Double
-- --
-- -- instance MonadSample m => MonadSample (StateCache c m) where
-- --   primitive = lift . primitive
-- --
-- -- instance MonadInfer m => MonadInfer (StateCache c m) where
-- --   factor = lift . factor
