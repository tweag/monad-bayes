...
data LeinProc =
  LeinProc Handle Handle ProcessHandle
...
anglican (LeinProc input output _) cmds
  -- execute command
 = do
...
startLein = do
  let setup =
        (shell "lein repl") {cwd = Just anglicanPath, std_in = CreatePipe, std_out = CreatePipe}
...
runAlg model (SMC n) =
  show <$> runPopulation (smcSystematic (modelLength model) n (buildModel model))
runAlg model (RMSMC n t) =
  show <$> runPopulation (rmsmcLocal (modelLength model) n t (buildModel model))
...
prepareBenchmark _ WebPPL model alg = bench name $ whnfIO run
  where
    ...
    dataString (LDA docs) =
      unwords $ zipWith (\x y -> "--doc" ++ show i ++ "='" ++ unwords doc ++ "'") [1 .. 5] docs
    run = do
      let command = "node " ++ webpplModelName model ++ ".js " ++ algString alg ++ dataString model
      (_, _, _, process) <-
        createProcess $
        (shell command) {cwd = Just webpplPath, std_out = NoStream, std_err = NoStream}
...
lengthBenchmarks e lrData hmmData ldaData = benchmarks
  where
    ...
    models =
      ...
      map (HMM . (`take` hmmData)) hmmLengths ++ map (\n -> LDA $ map (take n) ldaData) ldaLengths
...
samplesBenchmarks e lrData hmmData ldaData = benchmarks
  where
    ...
    algs =
      map (\x -> MH (100 * x)) [1 .. 10] ++
      map (\x -> SMC (100 * x)) [1 .. 10] ++ map (\x -> RMSMC 10 (10 * x)) [1 .. 10]
    benchmarks = map (uncurry3 (prepareBenchmark e)) $ filter supported xs
      where
        uncurry3 f (x, y, z) = f x y z
        xs = do
...
main = do
  ...
  let configSamples =
        defaultConfig {csvFile = Just "speed-samples.csv", rawDataFile = Just "raw.dat"}
