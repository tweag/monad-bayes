...
```haskell
tui 1 program showHistogram
```

This runs MCMC on `program` with 1 step throw away (burn-in). The algorithm will be displayed running in real time, with a progress bar and histogram:
