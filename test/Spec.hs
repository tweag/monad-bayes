...
main =
  hspec $ do
    ...
    describe "Dist" $ do
      it "sorts samples and aggregates weights" $ TestEnumerator.passed2 `shouldBe` True
      it "gives correct answer for the sprinkler model" $ TestEnumerator.passed3 `shouldBe` True
      it "computes expectation correctly" $ TestEnumerator.passed4 `shouldBe` True
    describe "Population" $ do
      context "controlling population" $ do
        ...
        it "correctly computes population average" $ TestPopulation.popAvgCheck `shouldBe` True
    ...
    describe "Sequential" $ do
      ...
      it "preserves the distribution" $ TestSequential.checkPreserve `shouldBe` True
    ...
    describe "SMC" $ do
      it "terminates" $ seq TestInference.checkTerminateSMC () `shouldBe` ()
      it "preserves the distribution on the sprinkler model" $
        TestInference.checkPreserveSMC `shouldBe` True
      prop "number of particles is equal to its second parameter" $ \observations particles ->
