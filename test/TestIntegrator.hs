...
passed6 = 
    ...
    ==
   sortOn fst (enumerateWith (fromList [1,2,3]) $ normalize $ do
...
passed7 = 
  ...
  ~==
  sortOn fst (enumerateWith (fromList [True, False]) $ normalize do
    x <- bernoulli 0.5
...
passed8 = 
  1 
  == 
  (volume $ fmap (ln . exp . snd) $ runWeighted do
  x <- bernoulli 0.5
...
model1 = do
  ...
  where f x = cos (x**4) + x ** 3
  
passed15 =
  (runIntegrator (ln . exp . snd) $ runWeighted do
  x <- bernoulli 0.5
  score 0.5
  return x) == 0.5
