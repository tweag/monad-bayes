module TestSequentialT (twoSync, finishedTwoSync, checkTwoSync, checkPreserve, pFinished, isFinished, checkSync) where
...
import Control.Monad.Bayes.SequentialT (advance, finish, finished)
import Data.AEq (AEq ((~==)))
