...
smc :: MonadDist m => Int -> Int ->
  ParticleT (EmpiricalT m) a -> EmpiricalT m a
smc k n d =
  flatten $ applyN k step $ start where
    start = lift (spawn n) >> d
    step = mapMonad (resampleN n) . advance
    applyN k f = foldr (.) id (replicate k f)
...
newtype MHKernel m a = MHKernel
        {runMHKernel :: a -> m (a,LogFloat)}
...
mh n init trans =
  evalStateT (start >>= chain n) 1 where
...
pimh :: MonadDist m => Int -> Int -> Int ->
  ParticleT (EmpiricalT m) a -> m [a]
pimh k np ns d =
  mhPrior ns $ transform $ smc k np d
