...
-- |
-- Module      : Control.Monad.Bayes.SequentialT
...
-- useful for implementation of SequentialT Monte Carlo related methods.
...
instance MonadSample m => MonadSample (SequentialT m) where
  random = lift random
  bernoulli = lift . bernoulli
  categorical = lift . categorical

-- | Execution is 'suspend'ed after each 'score'.
instance MonadCond m => MonadCond (SequentialT m) where
  score w = lift (score w) >> suspend

instance MonadInfer m => MonadInfer (SequentialT m)
...
-- | SequentialT importance sampling.
