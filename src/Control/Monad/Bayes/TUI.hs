...
module Control.Monad.Bayes.TUI where
...
import Control.Monad.Bayes.Sampler (SamplerIO, sampleIO, toBins)
import Graphics.Vty
import Brick.Widgets.Border
import Brick.Widgets.Center
import Brick.Widgets.Border.Style
import GHC.Float (double2Float)
import Control.Monad.Bayes.Inference.MH (runTraced)
...
tui :: Show a =>
    P.Producer (MHResult a) (Weighted SamplerIO) ()
    -> ([a] -> Widget ()) -> IO ()
tui producer visualizer = void do

  eventChan <- B.newBChan 10
  initialVty <- buildVty
  _ <- forkIO $ run (producer) eventChan n
