...
data SMCConfig m n = SMCConfig
  { resampler :: forall x. Population m n x -> Population m n x,
    numSteps :: Int,
    numParticles :: Int
  }
...
smcPush ::
  MonadInfer n m => SMCConfig m n -> Sequential (Population m) n a -> Population m n a
