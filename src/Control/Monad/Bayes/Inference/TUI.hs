...
import Control.Monad.Bayes.Inference.MCMC (runTraced)
import qualified Brick.BChan as B
import Control.Monad (void)
import Control.Concurrent (forkIO)
import Pipes ((>->), runEffect, MonadIO (liftIO))
import Data.List (sort)
import Control.Monad.Bayes.Traced.Common
    ( Trace(output, density), MHResult(success, trace) )
import Numeric.Log (Log(ln))
import Control.Monad.Bayes.Enumerator (toEmpirical)
import Text.Pretty.Simple (pShow, pShowNoColor)
import qualified Data.Text.Lazy as TL
import Control.Arrow (Arrow(..))
import Data.Scientific (fromFloatDigits, FPFormat (Exponent), formatScientific)
...
-- | a Viewer displays your data for the TUI to show
type Viewer a =  [a] -> Widget ()


noVisual :: Viewer a
noVisual = const emptyWidget

showEmpirical :: (Show a, Ord a) => Viewer a
showEmpirical = 
    txt . 
    T.pack . 
    TL.unpack . 
...
showVal :: Show a => Viewer a
showVal = txt . T.pack . (\case [] -> ""; a -> show $ head a)

showHistogram :: Viewer Double
...
drawUI handleSamples state = [ui] where
  ...
  completionBar = updateAttrMap
          (B.mapAttrNames [ (doneAttr, B.progressCompleteAttr)
  ...
  likelihoodBar = updateAttrMap
          ...
          ) $ B.progressBar (Just $ "Mean likelihood for last 1000 samples: " <> take 10 (show (head $ lk state <> [0])))
...
-- | runs the TUI
tui :: Show a => Traced (Weighted SamplerIO) a -> ([a] -> Widget ()) -> IO ()
tui distribution visualizer = void do

  eventChan <- B.newBChan 10
  initialVty <- buildVty
  _ <- forkIO $ run (runTraced 0 distribution) eventChan n
  samples <- B.customMain 
    initialVty 
    buildVty 
    (Just eventChan) 
    ((B.App { B.appDraw = drawUI visualizer
    ...
    (initialState n)
  TL.writeFile "src/data/tui_output.txt" (pShowNoColor samples)
  return samples
  ...
  run prod chan i = runEffect $
      P.hoist (sampler . unweighted) prod
      >-> P.scan
        (\mcmcdata@(MCMCData ns nsc smples lk _) a ->
          mcmcdata {
            numSteps = ns + 1,
            numSuccesses = nsc + if success a then 1 else 0,
            samples = output (trace a) : smples, lk = exp (ln (density (trace a))) : lk
              })
        (initialState i) 
        id 
      >-> P.take i
